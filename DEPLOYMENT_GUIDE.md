...
Ollin은 **자동화된 배포 스크립트**를 제공합니다. 이 스크립트는 버전 업데이트부터
배포까지 전체 과정을 자동화합니다.
...
스크립트가 대화형으로 진행되며, 다음을 자동으로 처리합니다:
...
4. 사전 검사 실행:
   ...
   - ESLint 검사
   - 빌드 검증
5. `package.json` 및 `app/manifest.json` 버전 업데이트
6. Git 커밋 생성
7. Git 태그 생성
8. 원격 저장소 푸시 (선택)
...
```bash
# 패치 버전 (버그 수정): 0.6.0 → 0.6.1
npm run deploy:patch

# 마이너 버전 (기능 추가): 0.6.0 → 0.7.0
npm run deploy:minor

# 메이저 버전 (큰 변경): 0.6.0 → 1.0.0
...
버전 업데이트 타입을 선택하세요:
  1) patch - 버그 수정 (0.6.0 → 0.6.1)
  2) minor - 기능 추가 (0.6.0 → 0.7.0)
  3) major - 큰 변경사항 (0.6.0 → 1.0.0)
...
✅ Updated package.json to v0.7.0
✅ Updated app/manifest.json to v0.7.0

📤 Git operations...
✅ Files staged
✅ Commit created: "chore: Bump version to 0.7.0"
✅ Tag created: v0.7.0
...
🚀 Pushing to remote...
✅ Commits pushed
✅ Tag pushed - GitHub Release will be created automatically

✨ Deployment complete!

📦 Check your GitHub Release: https://github.com/bearholmes/ollin/releases/tag/v0.7.0
...
- ✅ **자동 버전 관리**: package.json과 manifest.json 동시 업데이트
- ✅ **사전 검증**: 테스트, 린트, 빌드 자동 실행
- ✅ **안전성**: 문제 발생 시 배포 중단
- ✅ **일관성**: 커밋 메시지 및 태그 형식 통일
- ✅ **편리성**: 한 번의 명령으로 전체 배포 프로세스 완료
...
- [ ] 배포 스크립트 실행 (`npm run deploy`)
  - 스크립트가 자동으로 처리:
    - ✅ 테스트 실행
    - ✅ ESLint 검사
    - ✅ TypeScript 타입 체크
    - ✅ 빌드 실행
    - ✅ 버전 업데이트 (package.json + manifest.json)
    - ✅ Git 커밋 및 태그 생성
    - ✅ 원격 푸시 (선택)
